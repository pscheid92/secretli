...
async function deriveKeys(keyBytes: Uint8Array): Promise<{
  ...
  const publicID = new Uint8Array(
    await deriveSubkey(baseKey, 'share_item_uuid', 16),
  )
  const retrievalToken = new Uint8Array(
    await deriveSubkey(baseKey, 'share_item_token', 16),
  )
